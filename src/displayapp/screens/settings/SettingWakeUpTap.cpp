...
namespace {
  static void event_handler(lv_obj_t* obj, lv_event_t event) {
    SettingWakeUpTap* screen = static_cast<SettingWakeUpTap*>(obj->user_data);
    screen->UpdateSelected(obj, event);
  }
}

SettingWakeUpTap::SettingWakeUpTap(Pinetime::Applications::DisplayApp* app, Pinetime::Controllers::Settings& settingsController)
  ...
  lv_label_set_text_static(title, "Wake Up Tap");
...
SettingWakeUpTap::~SettingWakeUpTap() {
  lv_obj_clean(lv_scr_act());
  settingsController.SaveSettings();
}

void SettingWakeUpTap::UpdateSelected(lv_obj_t* object, lv_event_t event) {
  ...
  if (event == LV_EVENT_VALUE_CHANGED) {
    ...
    for (int index = 0; index < optionsTotal; ++index) {
      if (cbOption[index] == object) {
        ...
        switch (index) {
          case 0:
            settingsController.setWakeUpMode(WakeUpMode::SingleTap, false);
            settingsController.setWakeUpMode(WakeUpMode::DoubleTap, false);
            break;
          case 1:
            settingsController.setWakeUpMode(WakeUpMode::SingleTap, true);
            settingsController.setWakeUpMode(WakeUpMode::DoubleTap, false);
            break;
          case 2:
            settingsController.setWakeUpMode(WakeUpMode::SingleTap, false);
            settingsController.setWakeUpMode(WakeUpMode::DoubleTap, true);
            break;
        }
      } else {
