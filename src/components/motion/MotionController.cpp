...
namespace {
  ...
  int16_t DegreesRolled(int16_t y, int16_t z, int16_t prevY, int16_t prevZ) {
    int16_t prevYAngle = _lv_trigo_asin(Clamp(prevY * 32, -32767, 32767));
    int16_t yAngle = _lv_trigo_asin(Clamp(y * 32, -32767, 32767));
...
bool MotionController::ShouldRaiseWake() const {
  constexpr uint32_t varianceThresh = 48 * 48;
  ...
  AccelStats stats = GetAccelStats();
...
bool MotionController::ShouldLowerSleep() const {
  if (yHistory[2] < yHistory[1] + 192 || yHistory[2] < 512) {
    return false;
  }

  for (uint8_t i = 3; i < histSize; i++) {
    if (yHistory[i] < 0) {
      return false;
    }
  }
